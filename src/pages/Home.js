...
const Home = () => {
  ...
  const trendingData = useSelector(state => state.nrlflixData.bannerData);

  const { data: nowPlayingData } = useFetch("/movie/now_playing");
  const { data: topRatedData } = useFetch("/movie/top_rated");
  const { data: popularTvShowData } = useFetch("/tv/popular");
  const { data: onAirhowData } = useFetch("/tv/on_the_air");
  ...
  return (
    <div>
      ...
      <HorizontalScrollCard data={onAirhowData} heading={"On The Air"} media_type={"tv"} />
    </div>
  );
}
